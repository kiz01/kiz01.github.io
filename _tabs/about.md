---
title: About
icon: fas fa-user
order: 1
---

## About Me

I’m a Cloud Engineer focused on threat detection, system visibility, and practical security engineering.

My work centers on understanding how attacks affect Linux systems and building detection logic using tools like Wazuh. I approach learning through hands-on labs, real configurations, and controlled attack simulations.

This site documents my work: detection use cases, lab experiments, and system-level security analysis.

---

## What I Work On

**Cloud Engineering**
- Building hands-on experience with AWS (IAM, EC2, S3, VPC)
- Understanding cloud architecture, networking, and security fundamentals
- Creating lab environments to simulate real-world setups

**Detection Engineering (Wazuh)**
- Built and maintain a local Wazuh lab
- File Integrity Monitoring, log collection, and vulnerability detection
- Exploring detection logic, alert correlation, and system visibility

**Linux Security**
- System-level monitoring and configuration assessment
- Understanding persistence techniques and defensive controls

**Malware Analysis**
- Static and dynamic analysis using FLARE VM and REMnux
- Following structured workflows and documenting findings

**Security Operations (Foundation)**
- Alert triage and OSINT-based investigation
- Understanding attacker behavior and threat patterns

---

## How I Learn

I focus on building, testing, and observing systems to understand how they behave under attack.

Every concept is reinforced through hands-on labs, practical configurations, and detailed notes.

---

## What’s Next

- Strengthening AWS and Linux fundamentals  
- Building cloud-focused detection use cases  
- Advancing reverse engineering skills  
- Publishing structured, detection-focused write-ups  

---

## Connect

- **GitHub:** [kiz01](https://github.com/kiz01)
- **LinkedIn:** [Kshitiz Bhushan](https://linkedin.com/in/kiz01)

Feel free to reach out if you're working on similar things or want to discuss defensive security, cloud, or malware analysis.

